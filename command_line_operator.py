...
def run_analysis(video_pathNargs):
    video_path = video_pathNargs[0]
    args = video_pathNargs[1]
    output_dir,vidname = create_output_dir(video_path,args)
    print("Start processing video: ",video_path)
    video_parameters = pickle.load(open(os.path.join(args["dir_path"], "video_analysis_parameters.pickle"), 'rb'))[os.path.normpath(video_path)]
    video_parameters["starting_frame"] = args["starting_frame"] if args["starting_frame"] != None else video_parameters["starting_frame"]
...
if __name__ == '__main__':
    ...
    if not args["iter_dir"]:
        if args['collect_parameters']:
            ...
            CollectAnalysisParameters([args["vid_path"]], args["dir_path"], collect_starting_frame=args["collect_starting_frame"])
        ...
        if args['collect_parameters']:
            print("Collecting parameters for all videos in directory: ", args["dir_path"])
            CollectAnalysisParameters(videos_to_analyze, args["dir_path"], collect_starting_frame=args["collect_starting_frame"])
