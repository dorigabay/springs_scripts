...
if __name__ == "__main__":
    ...
    # data_analysis_dir_no_perspective_correction = f"Z:\\Dor_Gabay\\ThesisProject\\data\\3-data_analysis\\summer_2023\\experiment\\no_perspective_correction\\{spring_type}\\"
    ...
    video_dir_calib = f"Z:\\Dor_Gabay\\ThesisProject\\data\\1-videos\\summer_2023\\calibration\\{spring_type}\\"
    video_analysis_dir_calib = f"Z:\\Dor_Gabay\\ThesisProject\\data\\2-video_analysis\\summer_2023\\calibration\\{spring_type}\\"
    data_analysis_dir_calib = f"Z:\\Dor_Gabay\\ThesisProject\\data\\3-data_analysis\\summer_2023\\calibration\\{spring_type}\\"
    ...
    video_analysis_paths = video_analysis_paths[5:10]
    # ForceCalculator(video_dir, video_analysis_paths, data_analysis_dir, calibration_model_path)

    data_paths_calib = [root for root, dirs, files in os.walk(video_analysis_dir_calib) if not dirs]
    data_paths_calib = data_paths_calib[:1]
    ForceCalculator(video_dir, data_paths_calib, data_analysis_dir_calib, calibration_model_path, n_springs=1)
    print("-" * 60 + "\nTracking ants...\n"+"-"*20)
    ...
    # pool.join()
    print("-" * 60 + "\nAnalysing...\n" + "-" * 20)
    self = AnalyserPerSpring(data_analysis_dir_calib, spring_type)
