...
class Calculation(Ants):
    def __init__(self, parameters, image, previous_detections=None):
        ...
        self.max_ants_number = parameters["max_ants_number"]
    ...
    def occupied_springs(self, springs_order):
        ...
        labeled_ants_cropped = utils.crop_frame_by_coordinates(self.labeled_ants, self.object_crop_coordinates)
        ...
        for label in joints_labels:
            bundle_label = np.unique(self.bundles_labeled[dilated_ends == label])
            bundle_label = bundle_label[np.where(bundle_label != 0)]
            self.bundles_labeled[dilated_ends == label] = bundle_label[0]
    ...
    def reorder_coordinates(self, springs_order):
        fixed_ends_x, fixed_ends_y, free_ends_x, free_ends_y = \
            np.empty(self.n_springs), np.empty(self.n_springs), np.empty(self.n_springs), np.empty(self.n_springs)
        fixed_ends_x[:] = np.nan
        fixed_ends_y[:] = np.nan
        free_ends_x[:] = np.nan
        free_ends_y[:] = np.nan
        ...
        return fixed_ends_x.reshape(1, self.n_springs), fixed_ends_y.reshape(1, self.n_springs), free_ends_x.reshape(1,
            self.n_springs), free_ends_y.reshape(1, self.n_springs), blue_part_x.reshape(1, 2), blue_part_y.reshape(1, 2)
