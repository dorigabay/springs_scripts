...
class Calculation:
    ...
    def initiate_springs_angles_matrix(self,springs, previous_detections):
        ...
        elif previous_detections is None:
            self.previous_springs_angles = np.sort(springs.bundles_labels).reshape(1,self.n_springs)
        else:
            self.previous_springs_angles = previous_detections[-1]

    # def make_calculations(self,springs,ants):
    #     self.springs_angles_matrix = np.vstack([self.springs_angles_matrix, self.match_springs(springs, self.springs_angles_matrix[0, :])])
    #     springs_order = self.springs_angles_matrix[-1, :]
