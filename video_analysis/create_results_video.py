...
class TrackingResultVideo():
    ...
    def collect_cropping_coordinates(self,video_path):
        import pickle
        video_preferences_path = os.path.normpath("\\".join(video_path.split("\\")[:-2])+"_video_parameters.pickle")
    ...
    def present_results_on_frame(self, frame, frame_num):#,blobs_labels):
        ...
        # for count, spring in enumerate(assigned_ants):
        #     ant_label = count+1
        #     ant_coordinates = tracked_ants[tracked_ants[:, 2] == ant_label, :2]
        #     if ant_label == 1:
        #         print("ant_coordinates: ", ant_coordinates.shape, ant_coordinates)
        #     if ant_coordinates.shape[0] > 0:
        #         object_center = ant_coordinates.astype(int)[0]
...
if __name__ == "__main__":
    ...
    for i in [1]:
        ...
        data_path = f"Z:\\Dor_Gabay\\ThesisProject\\data\\3-data_analysis\\summer_2023\\plus0.1\\S5760001-S5760001\\"
        video_path = f"Z:\\Dor_Gabay\\ThesisProject\\data\\1-videos\\summer_2023\\13.8\\plus_0.1\\{vidname}.MP4"
        ...
        TrackingResultVideo(data_path, video_path, n_frames_to_save=1200)
